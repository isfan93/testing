...
class Rad_model extends CI_Model {
	...
	function get_antrian($lab_queue_status){
		...
		$this->db->where('substr(trx_lab_queue.lab_queue_id,1,2)','RD');
	...
	function get_diagnosa_support_detail($ds_id){
		...
		return $this->db->get_where('mst_lab_treathment_detail mltbd',array('mml.status'=>'1','mml.ds_id'=>$ds_id,'mltbd.dsd_status'=>'1','mlt.ds_type'=>'RAD'));
	}
