...
<script>
    $(function(){
        ...
        $('.btn-cancel-service').live('click',function(){
            if(confirm('Apakah layanan pasien benar - benar akan dibatalkan?'))
            {
                $.blockUI({
                    message: '<div class="black_loader"><img src="<?=get_loader(11)?>"></div>',
                    overlayCSS:  {
                    ...
                    $.unblockUI({
                        onUnblock: function(){
                            if (result.success === true) {
                                ...
                                alert('Layanan tidak dapat diakhiri, Silahkan hubungi administrator.');
                            }
                        }
                    });
                });
        ...
        $('ul#list_rekmed a[data-toggle="tab"]').click(function(e)
        {
            ...
            if(href != '#0'){
                $.get('<?=base_url()?>rawat_jalan/<?=$url_poli;?>/single_rekmed/'+service_id, function(data) {
                    $(href).html(data);
                });
            }else{
