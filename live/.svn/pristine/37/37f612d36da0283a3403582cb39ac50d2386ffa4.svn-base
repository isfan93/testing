...
<script type="text/javascript">
  function reloadTable(){
      var url_hasil="<?php echo base_url()?>master/data_user_group/loaddata_detail/"+$('#module_id').val();
      $(".list_treat_pack").load(url_hasil);
  }

  function resetInput(){
      //$('#lab_treat_name').val('');
      $("select[name='ds[module_id]']").val('');
      //$('#treat_lab_id').val('');
      //$('#treat_gol').val('');
  }
  $(function(){		
      ...
      $.ajax({
        url: '<?php echo base_url()?>master/data_user_group/loaddata_detail/'+$('#module_id').val(),
        type: "POST",
        crossDomain: true,
      ...
      $('.submit').click(function(){
          isvalid = $("form.tambahData").validate({
            rules: {
				...
				'ds[module_id]': "required",
            },
            submitHandler: function(form) {                
                formdata = $('form.tambahData').serialize();

                $(".black_loader").fadeIn(300).fadeOut(function(){
                  $.post( "<?=base_url()?>master/data_user_group/create_detail",formdata).done(function(){
...
<div class="container-fluid" style="padding:0px;">
  <div class="row-fluid">
      <div class="pageheader notab">
          <h1 class="pagetitle"><?=$konten['group_name']?><br><small><?//=$konten['module_name']?></small></h1>
          
      </div>
  </div>
  <br clear="all">
  <a class="btn btn-primary" style="float:right;" href="<?=base_url()?>master/data_user_group">back</a>
  <div class="page-content row-fluid">
    <form class="stdform form-horizontal tambahData" style="display:none" method="post" >
	  ...
	  <input type="hidden" name="ds[group_id]" id="module_id" value="<?= $konten['group_id']?>">
      <div class="span11">
        <div class="widget-box">
          ...
          <div class="controls">
            <select name='ds[module_id]' required id='group_id' >
				<option value="">-- Modul --</option>
				<?php foreach($home_base as $list):?>
					<option value="<?=$list->module_id?>"><?=$list->module_name?></option>
				<?php endforeach;?>
