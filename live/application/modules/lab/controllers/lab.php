...
class Lab extends MY_Controller {
	...
	function antrian()
	{
		$data['main_view']	= 'lab/antrian';
		$data['title']		= 'Instalasi Lab';
		$data['cf']			=  $this->cf;
		$data['current']	= '';
		$data['desc']		= 'Description. ';
		$data['antrian']	= $this->lab_model->get_antrian(array($this->activeAndPay,$this->active,$this->waitForPay,$this->payed));
		$this->load->view('template',$data);
	}
	...
	function data_diri($sd_rekmed = '', $mdc_id = '')
	{
		...
		if( ($sd_rekmed != '') && ($sd_rekmed != 0) )
		{
			...
			$this->db->where(array('substr(trx_lab_queue.lab_queue_id,1,2)'=>'LB','tv.visit_rekmed'=>$sd_rekmed));
		...
		}else if( $sd_rekmed == 0)
		{
			$this->db->_protect_identifiers=false;
			$this->db->join('trx_visit tv','tv.visit_id = substr(trx_lab_queue.lab_queue_id,4,8)');
			$this->db->where(array('substr(trx_lab_queue.lab_queue_id,1,2)'=>'LB', 'trx_lab_queue.lab_queue_id'=> $mdc_id));
			$this->db->where_in('lab_queue_status',array('1','2','3','4'));
			$data_pasien		= $this->db->get('trx_lab_queue');
			if($data_pasien->num_rows() >= 1){
	...
	function load_form($lab_queue_id){
		...
		$this->db->where_in('me.sd_type_user',array('18')); // 17 18 patologi klinis
		$data['dokter']		= $this->db->get('mst_employer me');

		$this->db->select('me.id_employe,me.sd_name as sd_name');
		$this->db->where_in('me.sd_type_user',array('28')); // 26 radi 28 lab
	...
	function genIdPenunjangDiagnosa($lab_queue_id){
		$format = str_replace('LB', 'PD', $lab_queue_id);
		...
		foreach ($data['penunjang'] as $key => $value) {
			if(!empty($value)){
				...
				if($exist->num_rows == 0){
					...
					$data_visit_detail['bill_id']	= str_replace('LB', 'BL', $lab_queue_id);
					...
					if($data_ds_detail->num_rows() >= 1){
						...
						foreach ($data_ds_detail->result() as $k => $v) {
							if(($v->dsd_standart_value_male=='')||($v->dsd_standart_value_female=='')){
								if($v->dsd_standart_value_male!=''){
									$nilai_rujukan=$v->dsd_standart_value_male;
								}else{
									$nilai_rujukan=$v->dsd_standart_value_female;
								}							
							}else{
								$nilai_rujukan = $v->dsd_standart_value_male.', '.$v->dsd_standart_value_female;
							}
							$dtd[] = array(
								'lab_queue_id' 	=> $lab_queue_id,
								...
								'dsupport_value'=> '',
								'dsupport_standart_value' => $nilai_rujukan,//'dsupport_standart_value' => 'L: '.$v->dsd_standart_value_male.', P: '.$v->dsd_standart_value_female,
	...
	function delete_list()
	{
		...
		if($lab_queue_id != null && $trx_ds_id != null && $ds_id != null && $ds_name != null)
		{
			...
			$this->db->where(array('bill_id'=>str_replace('LB', 'BL', $lab_queue_id),'service_type'=>'7','service_name'=>$ds_name,'payment_status'=>'0'));
	...
	function finish_pemeriksaan(){
		...
		foreach ($ket as $key2 => $value2) {
			$dtk['dsupport_ekspertise_note'] = $value2;
			$this->db->where(array('lab_queue_id'=>$lab_queue_id,'dsupport_code'=>$key2));
			$this->db->update('trx_lab_treathment_detail',$dtk);
			// unset($dt['dsupport_value_image']);
		}
	...
	public function pemeriksaan()
	{
		...
		$this->db->join('trx_service ts','tv.visit_id = substr(ts.service_id,4,8)');
		$this->db->join('ptn_social_data psd','tv.visit_rekmed = psd.sd_rekmed','left');
		$this->db->where(array('ts.service_status'=>'5','substr(ts.service_id,1,2)'=>'LB'));
	...
	public function daftar()
	{
		...
		$data['title']		= 'Pendaftaran Pemeriksaan Laboratorium';
		$data['cf']			=  $this->cf;
		$data['current']	= '';
		$data['desc']		= 'Description. ';
