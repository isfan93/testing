...
function reloadTable(){
    var url_hasil="<?php echo base_url()?>master/data_rute_obat/loaddata"
          ...
          if (r == true) {
                ...
                $(".black_loader").fadeIn(300).fadeOut(function(){
                  // console.log($(t).attr('href'));
                   $.ajax({
                      url: "data_rute_obat/delete_list",
      ...
      $('.modals').click(function(){
        ...
        $('.btn_utility').hide();
         ...
         $('.searchname').keyup(function(){
           ...
           $('.list_rute').children().remove();
           $.ajax({
                          url: '<?php echo base_url()?>master/data_rute_obat/loaddata',
                          type: "POST",
                          crossDomain: true,
         ...
         $('.submit_search').click(function(){
          ...
          isvalid = $("form.tambahData").validate({
            ...
            submitHandler: function(form) {
                ...
                $(".black_loader").fadeIn(300).fadeOut(function(){
                  $.post( "<?=base_url()?>master/data_rute_obat/create",formdata).done(function(){
           ...
           var r = confirm("Anda yakin akan menghapus data poli ini ?");
            ...
            return false;
              ...
              $('.submitEdit').live("click",function(){
                  ...
                  crsf = $('.crsf').val();
                  url = "<?=base_url()?>master/data_rute_obat/update_master";
                  datapost = "csrf_jike_2012="+crsf+"&mst[rute_id]="+id+"&mst[rute_name]="+name;
          ...
          <form class="stdform form-horizontal tambahData" style="display:none" method="post" >
            <input type="hidden" class='crsf' name="<?php echo $this->security->get_csrf_token_name()?>" value="<?php echo $this->security->get_csrf_hash()?>" >
              ...
              <div class="control-group" style="border:none;">
                  <label class="control-label" style="width:20%;">Nama rute</label>
                  <div class="controls" >
                      <input type="text" class='' required name="mst[rute_name]" id="nama_rute_obat">
                  </div>
              </div>
              ...
              <input type="hidden" class='crsf' name="<?php echo $this->security->get_csrf_token_name()?>" value="<?php echo $this->security->get_csrf_hash()?>" >
                <?=form_open(cur_url().'search',array('class' => 'form-horizontal frm_search stdform','id' => 'form')); ?> 
                    ...
                    <div class="chatsearch" >
                        <input type="text" class='searchname' name="nama_dokter" placeholder="Nama Rute Obat" style="wi dth:91%;margin:auto;">
                    </div>
