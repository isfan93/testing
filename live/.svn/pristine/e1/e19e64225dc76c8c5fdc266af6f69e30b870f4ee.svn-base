...
<script type="text/javascript">
  function reloadTable(){
      var url_hasil="<?php echo base_url()?>master/data_user_group/loaddata"
      $(".list_group").load(url_hasil);
  }

  function resetInput(){
      $('#ds_name').val('');
      $("select[name='ds[group_homebase]']").val('');
      //$('#ds_price').val('');
      //$('#im_max_stock').val('');
  }
  $(function(){
            ...
            if (r == true) {
                  ...
                  $(".black_loader").fadeIn(300).fadeOut(function(){
                    // console.log($(t).attr('href'));
                     $.ajax({
                        url: "<?php echo base_url()?>master/data_user_group/delete_list",
      ...
      $.ajax({
        url: '<?php echo base_url()?>master/data_user_group/loaddata',
        type: "POST",
        crossDomain: true,
      ...
      $('.submit').click(function(){
          isvalid = $("form.tambahData").validate({
            ...
            submitHandler: function(form) {
                ...
                $(".black_loader").fadeIn(300).fadeOut(function(){
                  $.post( "<?=base_url()?>master/data_user_group/create",formdata).done(function(){
      ...
      $('.submitEdit').live("click",function(){
          ...
          crsf = $('.crsf').val();
          url = "<?=base_url()?>master/data_user_group/update";
          datapost = "csrf_jike_2012="+crsf+"&ds[group_name]="+ds_name+"&ds[group_homebase]="+ds_type+"&ds[group_id]="+ds_id;
    ...
    <form class="stdform form-horizontal tambahData" style="display:none" method="post" >
      ...
      <div class="span11">
        <div class="widget-box">
          ...
          <div class="controls">
            <input type="text" autofocus="" name="ds[group_name]" id="ds_name" placeholder="Nama Group">
          </div>
		<label class="control-label">Home Base</label>
          <div class="controls">
            <select name='ds[group_homebase]' required id='home_base' >
				<option value="">-- Home Base --</option>
				<?php foreach($home_base as $list):?>
					<option value="<?=$list->module_url?>"><?=$list->module_name?></option>				
				<?php endforeach;?>
        ...
        <div class="widgetbox">
            ...
            <?=form_open(cur_url().'search',array('class' => 'form-horizontal frm_search stdform','id' => 'form')); ?>
              <div class="chatsearch" >
                  <input type="text" class='searchname' name="nama_dokter" placeholder="Cari User Group" style="wi dth:91%;margin:auto;">
              </div>
