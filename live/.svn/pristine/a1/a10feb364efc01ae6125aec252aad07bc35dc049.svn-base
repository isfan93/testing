...
<script type="text/javascript">
  function reloadTable(){
      var url_hasil="<?php echo base_url()?>master/data_tindakan_lab/loaddata"
      $(".list_obat").load(url_hasil);
  }

  function resetInput(){
      $('#ds_name').val('');
      $("select[name='ds[ds_type]']").val('');
      $('#ds_price').val('');
      //$('#im_max_stock').val('');
  }
  $(function(){
      ...
      $.ajax({
        url: '<?php echo base_url()?>master/data_tindakan_lab/loaddata',
        type: "POST",
        crossDomain: true,
      ...
      $('.submit').click(function(){
          isvalid = $("form.tambahData").validate({
            ...
            submitHandler: function(form) {
                ...
                //$(".black_loader").fadeIn(300).fadeOut(function(){
                  $.post( "<?=base_url()?>master/data_tindakan_lab/create",formdata).done(function(){
      ...
      $('.submitEdit').live("click",function(){
          t = this;
          ds_id =  $(this).parent().parent().parent().find('.ds_id').val();
          ds_name =  $(this).parent().parent().parent().find('.ds_name').val();
          ds_price =  $(this).parent().parent().parent().find('.ds_price').val();
          ds_type =  $(this).parent().parent().parent().find('.ds_type').val();
          ins_gol =  $(this).parent().parent().parent().find('.dsim_gol').val();
          
          crsf = $('.crsf').val();
          url = "<?=base_url()?>master/data_tindakan_lab/update";
          datapost = "csrf_jike_2012="+crsf+"&ds[ds_name]="+ds_name+"&ds[ds_price]="+ds_price+"&ds[ds_type]="+ds_type+"&ds[ds_id]="+ds_id+"&ds[ds_gol]="+ins_gol;;
      ...
      <div class="span11">
        <div class="widget-box">
          <div class="widget-title">
            ...
            <h5>Tambah Data Tindakan Lab</h5>
          </div>
          
          <label class="control-label">Nama Tindakan Lab</label>
          <div class="controls">
            <input type="text" autofocus="" name="ds[ds_name]" id="ds_name" placeholder="Nama Tindakan Lab">
          </div>
          <label class="control-label">Tipe</label>
          <div class="controls">
      			<select name="ds[ds_type]">
          ...
          <label class="control-label" >Golongan</label>
             <div class="controls">
               <select name='ds[ds_gol]' required id='ds_gol' >
                <option value="">-- Pilih --</option>
          ...
          <div class="controls">
            <input type="text" autofocus="" name="ds[ds_price]" id="ds_price" placeholder="Rp. 00,00">
          </div>
        ...
        <div class="widgetbox">
            ...
            <?=form_open(cur_url().'search',array('class' => 'form-horizontal frm_search stdform','id' => 'form')); ?>
              <div class="chatsearch" >
                  <input type="text" class='searchname' name="nama_dokter" placeholder="Cari Tindakan Laboratorium" style="wi dth:91%;margin:auto;">
              </div>
