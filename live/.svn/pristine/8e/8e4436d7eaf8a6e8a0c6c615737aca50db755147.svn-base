							...
							<?php
								...
								if( $dokter_fee->num_rows() >= 1 )
								{
									...
									<tr>
										...
										<!-- <td style="text-align:right;"><?php echo number_format($dokter_fee->dr_fee, 2, ",", ".");?></td> -->
										<td style="text-align:right;"><?php echo number_format($dokter_fee->dr_fee, 2, ",", ".");?></td>
									</tr>
									<?php
								...
								<?php if( (isset($visite)) && ($visite->num_rows()) ) : ?>
									...
									<?php foreach ($visite->result() as $key => $value) : ?>
										<tr>
											...
											<td style="text-align:right;"><?php echo number_format($value->treat_item_price, 2, ",", ".");?></td>
										</tr>
										<?php 
									...
									foreach ($adm_fee->result() as $key => $value) {
										?>
											<tr>
												...
												<!-- <td style="text-align:right;"><?=number_format($value->adm_fee, 2, ",", ".")?></td> -->
												<!-- <td style="text-align:right;">0</td> -->
												<td style="text-align:right;"><?=number_format(($value->adm_fee), 2, ",", ".")?></td>
											</tr>
										<?php
								...
								if( ($resep->num_rows()) >= 1 ){
									...
									<?php
									$i = 1;//$tuslah=$resep->num_rows()*1000;
									foreach ($resep->result() as $key => $value) {
										?>
											<tr>
												...
												<!-- <td style="text-align:right;"><?=number_format($value->im_item_price, 2, ",", ".")?></td> -->
												<!-- <td style="text-align:right;">0</td> -->
												...
												<td style="text-align:right;"><?=number_format( ($value->recipe_price * $value->recipe_qty), 2, ",", ".")?></td>
											</tr>
											
										<?php
										$i++;
										$total += $value->recipe_price * $value->recipe_qty;
										$subobat += $value->recipe_price * $value->recipe_qty;
									}
									//$total += $tuslah;
									?>
								...
								if( ($racikan->num_rows() >= 1) )
								{
									foreach ($racikan->result() as $key => $value) {
										...
										<tr>
											<td style="border-left:0px;text-align:center;width:20px;"><?=$i?></td>
											<td><?=$value->racikan_name?> <?=$value->racikan_rule?></td>
											<td style="text-align:right;"><?= number_format($value->racikan_total,2,",",".")?></td>
										</tr>
										<?php
										$i++;
										$total += $value->racikan_total;
										$subobat += $value->racikan_total;
									}
								}

								if(($tuslah->num_rows() >=1)){
									$hargatuslah = $tuslah->row_array();
									?>
									<tr>
										<td></td>
										<td><?= $hargatuslah['service_name']?></td>
										<td style="text-align:right;"><?=number_format( $hargatuslah['total_price'], 2, ",", ".")?></td>
									</tr>
									<?php
									$total += $hargatuslah['total_price'];
									$subobat += $hargatuslah['total_price'];
								}

								if((($racikan->num_rows() >=1))||(($resep->num_rows() >=1))){
									?>
									<tr>
										<td colspan="2" style="text-align:left;font-weight:bold;border-left:0px;">Sub Total Pembelian Obat & Alkes</td>
										<td style="text-align:right;"><b><?=number_format( $subobat, 2, ",", ".")?></b></td>
									</tr>
									<?php
								}
									...
									foreach ($tindakan->result() as $key => $value) {
										?>
											<tr>
												...
												<!-- <td style="text-align:right;"><?=number_format(($value->treat_item_price), 2, ",", ".")?></td> -->
												<!-- <td style="text-align:right;">0</td> -->
												<td style="text-align:right;"><?=number_format($value->treat_item_price, 2, ",", ".")?></td>
											</tr>
										<?php
										$i++;
										$total += $value->treat_item_price;
										$subtindakan += $value->treat_item_price;
									}
								}
								...
								if(($tindakan->num_rows()) >= 1){
									?>
									<tr>
										<td colspan="2" style="text-align:left;font-weight:bold;border-left:0px;">Sub Total Tindakan</td>
										<td style="text-align:right;"><b><?=number_format( $subtindakan, 2, ",", ".")?></b></td>
									</tr>
									<?php
								}
							?>
