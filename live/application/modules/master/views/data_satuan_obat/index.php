...
function reloadTable(){
            var url_hasil="<?php echo base_url()?>master/data_satuan_obat/loaddata"
            $(".list_tindakan").load(url_hasil);
}

function resetInput(){
          $('#nama_satuan_obat').val('');
          ...
          if (r == true) {
                ...
                $(".black_loader").fadeIn(300).fadeOut(function(){
                  // console.log($(t).attr('href'));
                   $.ajax({
                      url: "data_satuan_obat/delete_list",
      ...
      $('.modals').click(function(){
        ...
        jQuery('.tgl_frm').val(getDateRaw(0)); 
          ...
          crsf = $('.crsf').val();
           ...
           $.ajax({
                          url: '<?php echo base_url()?>master/data_satuan_obat/loaddata',
                          type: "POST",
                          crossDomain: true,
         ...
         $('.submit_search').click(function(){
          ...
          isvalid = $("form.tambahData").validate({
            ...
            submitHandler: function(form) {
                ...
                $(".black_loader").fadeIn(300).fadeOut(function(){
                  $.post( "<?=base_url()?>master/data_satuan_obat/create",formdata).done(function(){
           ...
           var r = confirm("Anda yakin akan menghapus data poli ini ?");
            ...
            return false;
              ...
              $('.submitEdit').live("click",function(){
                  ...
                  crsf = $('.crsf').val();
                  url = "<?=base_url()?>master/data_satuan_obat/update_master";
                  datapost = "csrf_jike_2012="+crsf+"&mst[mtype_id]="+id+"&mst[mtype_name]="+name;
          ...
          <form class="stdform form-horizontal tambahData" style="display:none" method="post" >
            <input type="hidden" class='crsf' name="<?php echo $this->security->get_csrf_token_name()?>" value="<?php echo $this->security->get_csrf_hash()?>" >
              ...
              <div class="control-group" style="border:none;">
                                <label class="control-label" style="width:20%;">Nama Satuan</label>
                                <div class="controls" >
                                    <input type="text" class='' required name="mst[mtype_name]" id="nama_satuan_obat">
                                </div>
                            </div>
              ...
              <input type="hidden" class='crsf' name="<?php echo $this->security->get_csrf_token_name()?>" value="<?php echo $this->security->get_csrf_hash()?>" >
                <?=form_open(cur_url().'search',array('class' => 'form-horizontal frm_search stdform','id' => 'form')); ?> 
                    ...
                    <div class="chatsearch" >
                        <input type="text" class='searchname' name="nama_dokter" placeholder="Nama Satuan Obat" style="wi dth:91%;margin:auto;">
                    </div>
