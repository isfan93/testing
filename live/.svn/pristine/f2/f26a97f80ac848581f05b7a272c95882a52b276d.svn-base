...
class Poli_model extends CI_Model {
	...
	function get_antrian_active_poli($pl_id){
		$this->db->select('tqo.*,psc.*,ts.*');
		$this->db->where('tqo.pl_id',$pl_id);
		$this->db->where(array('tqo.queo_status'=>1));
		$this->db->join('ptn_social_data psc','tqo.sd_rekmed = psc.sd_rekmed');
		$this->db->join('trx_service ts','ts.service_id = tqo.queo_id');
		$result = $this->db->get('trx_queue_outpatient tqo');
		return $result;
	}
	...
	function get_trx_penunjang_detail_rekmed($mdc_id)
	{
		// $visit_id = substr($mdc_id, 3,8);
		// $this->db->join('mst_lab_treathment mt','mt.ds_id = tltd.ds_id');
		// $this->db->like('tltd.lab_queue_id',$visit_id);
		// return $this->db->get('trx_lab_treathment_detail tltd');
	}
	...
	{
		$this->db->select('tdt.mdc_id,tdtd.dat_id,tdtd.dat_treat,mt.treat_name,mt.treat_item_price');
	...
	{
		$this->db->select('tdt.mdc_id,tdt.dat_id,tdt.dat_note,tdt.dat_case,tdt.dat_diag,md.diag_name,md.description,md.indonesian,md.diag_kode');
		$this->db->where('tdt.mdc_id',$mdc_id);
	...
	function get_racikan_detail($racikan_id)
	{
		return $this->db->get_where('trx_racikan_detail',array('racikan_id'=>$racikan_id))->result();
	}

	function get_tuslah_fee($mdc_id){
		$temp = $this->db->where('service_id',$mdc_id)->get('trx_service')->row_array();
		$key = $temp['bill_id'];
		return $this->db->get_where('trx_bill_detail',array('bill_id'=>$key,'service_type' => 10));
	}
