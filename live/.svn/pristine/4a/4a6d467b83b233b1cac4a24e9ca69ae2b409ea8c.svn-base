...
<script type="text/javascript">
  function reloadTable(){
      var url_hasil="<?php echo base_url()?>master/data_adm_fee/loaddata"
  ...
  $(function(){
            ...
            if (r == true) {
                  ...
                  $(".black_loader").fadeIn(300).fadeOut(function(){
                    // console.log($(t).attr('href'));
                     $.ajax({
                        url: "<?php echo base_url()?>master/data_adm_fee/delete_list",
      ...
      $.ajax({
        url: '<?php echo base_url()?>master/data_adm_fee/loaddata',
        type: "POST",
        crossDomain: true,
      ...
      $('.submit').click(function(){
          isvalid = $("form.tambahData").validate({
            ...
            submitHandler: function(form) {                
                ...
                $(".black_loader").fadeIn(300).fadeOut(function(){
                  $.post( "<?=base_url()?>master/data_adm_fee/create",formdata).done(function(){
      ...
      $('.submitEdit').live("click",function(){
          t = this;
          ins_id =  $(this).parent().parent().parent().find('.adm_fee_id').val();
          ins_name =  $(this).parent().parent().parent().find('.dsim_name').val();
		  ins_fee =  $(this).parent().parent().parent().find('.dsim_fee').val();
          
          crsf = $('.crsf').val();
          url = "<?=base_url()?>master/data_adm_fee/update";
          datapost = "csrf_jike_2012="+crsf+"&ds[adm_name]="+ins_name+"&ds[adm_id]="+ins_id+"&ds[adm_fee]="+ins_fee;
    ...
    <form class="stdform form-horizontal tambahData" style="display:none" method="post" >
      ...
      <div class="span11">
        <div class="widget-box">
          ...
          <label class="control-label">Jenis Administrasi</label>
          <div class="controls">
            <input type="text" autofocus="" name="ds[adm_name]" id="adm_name" placeholder="Jenis Administrasi">
          </div>
		  <label class="control-label">Tarif</label>
          <div class="controls">
            <input type="text" autofocus="" name="ds[adm_fee]" id="adm_fee" placeholder="Tarif">
          </div>
        ...
        <div class="widgetbox">
            ...
            <?=form_open(cur_url().'search',array('class' => 'form-horizontal frm_search stdform','id' => 'form')); ?>
              <div class="chatsearch" >
                  <input type="text" class='searchname' name="nama_dokter" placeholder="Cari Jenis Administrasi" style="wi dth:91%;margin:auto;">
              </div>
