...
<script type="text/javascript">
  function reloadTable(){
    var url_hasil="<?php echo base_url()?>master/data_room/loaddata"
  ...
  function resetInput(){
			...
			$("select[name='mst[pavillion_id]']").val('');
            //$('#im_min_stock').val('');
            //$('#im_max_stock').val('');
  }
  jQuery(document).ready(function() {
    ...
    $('.btn_aktif').live("click",function(){
            // alert('asd');
            t = this;
           var r = confirm("Anda yakin akan mengaktifkan room ini ?");
              if (r == true) {
       ...
       var r = confirm("Anda yakin akan menghapus data ini ?");
          if (r == true) {
                ...
                $(".black_loader").fadeIn(300).fadeOut(function(){
                  // console.log($(t).attr('href'));
                   $.ajax({
                      url: "<?php echo base_url()?>master/data_room/delete_list",
      ...
      $('.modals').click(function(){
        ...
        jQuery('.tgl_frm').val(getDateRaw(0)); 
          ...
          crsf = $('.crsf').val();
           ...
           $.ajax({
                          url: '<?php echo base_url()?>master/data_room/loaddata',
                          type: "POST",
                          crossDomain: true,
          ...
          isvalid = $("form.tambahData").validate({
            rules: {
                'ds[room_number]': {"required": true, "number": true},
            ...
            submitHandler: function(form) {
              ...
              //  var data = $("#form_pendaftaran_baru").serialize();
                ...
                $(".alert").fadeIn(800).fadeOut(function(){
                  $.post( "<?= base_url()?>master/data_room/create",formdata).done(function(){
                     $('.tambahData').slideUp();
                      $('.btn_utility').show();
                      reloadTable();
                      resetInput();
         ...
         $('.add_schdl').click(function(){
          return false;
           ...
           $('.btn_trash').live("click",function(){
            // alert('asd');
            t = this;
           var r = confirm("Anda yakin akan menon-aktifkan data ini ?");
              if (r == true) {
                $(".black_loader").fadeIn(300).fadeOut(function(){
                  // console.log($(t).attr('href'));
                  $.get($(t).attr('href')).done(function(){
            ...
            return false;
              ...
              $('.submitEdit').live("click",function(){
                  t = this;
                  im_room =  $(this).parent().parent().parent().find('.dsim_room').val();
                  im_room_number =  $(this).parent().parent().parent().find('.dsim_rn').val();
                  ...
                  crsf = $('.crsf').val();
                  url = "<?=base_url()?>master/data_room/update";
                  datapost = "csrf_jike_2012="+crsf+"&ds[room_id]="+im_room+"&ds[class_id]="+im_class+"&ds[pavillion_id]="+im_pav+"&ds[room_number]="+im_room_number;
        ...
        <div class="widget-box">
          <div class="widget-title">
            ...
            <h5>Tambah Data</h5>
				...
				<div class="controls" >
					<input type="text" class='' required name="mst[room_number]" id="room_id">
					<!--select name="mst[room_number]" id="room_id">
						<?php //for($i=1;$i<31;$i++):?>
				 ...
				 <label class="control-label" >Nama Pavillon</label>
				 <div class="controls">
					 <select name='mst[pavillion_id]' required id='tipe_pavillion' >
						<option value="">-- Pilih --</option>
						<?php foreach($tipe_pavillion as $rows):?>
						<option value="<?=$rows->pavillion_id?>"><?=$rows->pavillion_name?></option>
						
						<?php endforeach;?>
						
					</select>
				</div>
				<label class="control-label" >Tipe Class</label>
				 <div class="controls">
					 <select name='mst[class_id]' required id='tipe_class' >
						<option value="">-- Pilih --</option>
						<?php foreach($tipe_class as $rows):?>
						<option value="<?=$rows->class_id?>"><?=$rows->class_name?></option>
