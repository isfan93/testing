								...
								<?php foreach ($racikan_medicine_detail[$value->racikan_id] as $k => $v) : ?>
									<tr style="background-color:#FFFFFF !important;" class="detail_<?=$value->racikan_id?>">
										<td colspan="3" style="background-color:#FFFFFF !important;border-top:none;">
											...
											<input type="hidden" name="racikan_medicine_summary[]" class="recipe_summary" id="racikan_medicine_summary_<?=$key?>_<?=$k?>" recipe_price="<?=$v->racikan_medicine_price?>" recipe_qty="<?=(float) $v->racikan_medicine_qty?>" recipe_medicine="<?=$v->racikan_medicine?>" recipe_adm="0" />
										</td>
									</tr>
									<script type="text/javascript">
										var i = "<?=$k?>";
							...
							<?php for ($k= $k+1; $k <= 7 ; $k++) : ?>
								<tr style="background-color:#FFFFFF !important;" class="detail_<?=$value->racikan_id?>">
									<td colspan="3" style="background-color:#FFFFFF !important;border-top:none;">
										...
										<input type="hidden" name="racikan_medicine_summary[]" class="recipe_summary" id="racikan_medicine_summary_<?=$key?>_<?=$k?>" recipe_price="" recipe_qty="" recipe_medicine="" recipe_adm="0" />
									</td>
								</tr>
								<script type="text/javascript">
									var i = "<?=$k?>";
