...
<script type="text/javascript">
  function reloadTable(){
      var url_hasil="<?php echo base_url()?>master/data_treat_pack/loaddata"
  ...
  $(function(){
            ...
            if (r == true) {
                  ...
                  $(".black_loader").fadeIn(300).fadeOut(function(){
                    // console.log($(t).attr('href'));
                     $.ajax({
                        url: "<?php echo base_url()?>master/data_treat_pack/delete_list",
      ...
      $.ajax({
        url: '<?php echo base_url()?>master/data_treat_pack/loaddata',
        type: "POST",
        crossDomain: true,
      ...
      $('.submit').click(function(){
          isvalid = $("form.tambahData").validate({
            ...
            submitHandler: function(form) {                
                ...
                $(".black_loader").fadeIn(300).fadeOut(function(){
                  $.post( "<?=base_url()?>master/data_treat_pack/create",formdata).done(function(){
      ...
      $('.submitEdit').live("click",function(){
          t = this;
          ins_id =  $(this).parent().parent().parent().find('.pt_id').val();
          ins_name =  $(this).parent().parent().parent().find('.dsim_name').val();
		  ins_desc =  $(this).parent().parent().parent().find('.dsim_desc').val();
          
          crsf = $('.crsf').val();
          url = "<?=base_url()?>master/data_treat_pack/update";
          datapost = "csrf_jike_2012="+crsf+"&ds[pt_name]="+ins_name+"&ds[pt_id]="+ins_id+"&ds[pt_desc]="+ins_desc;
    ...
    <form class="stdform form-horizontal tambahData" style="display:none" method="post" >
      ...
      <div class="span11">
        <div class="widget-box">
          ...
          <label class="control-label">Nama Paket Tindakan</label>
          <div class="controls">
            <input type="text" autofocus="" name="ds[pt_name]" id="pt_name" placeholder="Nama Paket Tindakan">
          </div>
		  <label class="control-label">Deskripsi</label>
          <div class="controls">
            <input type="text" autofocus="" name="ds[pt_desc]" id="pt_desc" placeholder="Deskripsi">
          </div>
