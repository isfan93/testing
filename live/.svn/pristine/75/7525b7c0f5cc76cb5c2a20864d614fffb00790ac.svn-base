...
<script type="text/javascript">
  function reloadTable(){
      var url_hasil="<?php echo base_url()?>master/data_koefisien_fee/loaddata"
  ...
  $(function(){
      ...
      crsf = $('.crsf').val();
  		...
  		$(".delete_data").on("click",function(e){
  			...
  			if(id_array!=0){
         var r = confirm("Anda yakin akan menghapus data askes ini ?");
            if (r == true) {
          
                  $(".black_loader").fadeIn(300).fadeOut(function(){
                    // console.log($(t).attr('href'));
                     $.ajax({
                        url: "<?php echo base_url()?>master/data_koefisien_fee/delete_list",
      ...
      $.ajax({
        url: '<?php echo base_url()?>master/data_koefisien_fee/loaddata',
        type: "POST",
        crossDomain: true,
      ...
      $('.submit').click(function(){
          isvalid = $("form.tambahData").validate({
            rules: {
                'ds[koef_name]': "required",              
            },
            submitHandler: function(form) {                
                formdata = $('form.tambahData').serialize();

                $(".black_loader").fadeIn(300).fadeOut(function(){
                  $.post( "<?=base_url()?>master/data_koefisien_fee/create",formdata).done(function(){
      ...
      $('.submitEdit').live("click",function(){
          t = this;
          koef_id =  $(this).parent().parent().parent().find('.koef_id').val();
          koef_name =  $(this).parent().parent().parent().find('.dsim_name').val();
          koef_value =  $(this).parent().parent().parent().find('.dsim_value').val();
          crsf = $('.crsf').val();
          url = "<?=base_url()?>master/data_koefisien_fee/update";
          datapost = "csrf_jike_2012="+crsf+"&ds[koef_treathment]="+koef_name+"&ds[koef_id]="+koef_id+"&ds[koef_value]="+koef_value;
    ...
    <form class="stdform form-horizontal tambahData" style="display:none" method="post" >
      ...
      <div class="span5">
        <div class="widget-box">
          ...
          <label class="control-label">Kategori Tindakan</label>
          <div class="controls">
            <input type="text" autofocus="" name="ds[koef_treathment]" id="koef_name" placeholder="Kategori Tindakan">
          </div>
		  <label class="control-label">Nilai Koefisien</label>
		  <div class="controls">
            <input type="text" autofocus="" name="ds[koef_value]" id="koef_name" placeholder="Nilai Koefisien"><br>
			<small>*gunakan titik (.) sebagai pengganti koma (,)</small>
          </div>
        ...
        <div class="widgetbox">
            ...
            <?=form_open(cur_url().'search',array('class' => 'form-horizontal frm_search stdform','id' => 'form')); ?>
              <div class="chatsearch" >
                  <input type="text" class='searchname' name="nama_dokter" placeholder="Cari kategori tindakan" style="width:91%;margin:auto;">
              </div>
              <div style='display:none;' id="basic">
                  <input type="text" class="mediuminput" placeholder="masukkan nama kategori tindakan" autofocus="autofocus">
              </div>
