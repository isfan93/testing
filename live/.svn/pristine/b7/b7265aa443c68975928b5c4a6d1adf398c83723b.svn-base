<script type="text/javascript">
	...
	$(function(){
	          			...
	          			if( $('#medicine_'+i).select2('data') != null )
	          			{
		          			if( e.val == $('#medicine_'+i).select2('data').id )
		          			{
		          				...
		          				if( ((result.kel_name) != null) && (result.kel_name == 'Tuslah') )
		          					...
		          					recipe_adm = 0; // Pembelian Umum tanpa biaya administrasi
	          					...
	          					$("#recipe_summary_"+i).attr("recipe_adm",recipe_adm);	
		...
		<?php if(isset($pembelian_umum_medicine)) : ?>
			<?php if(($pembelian_umum_medicine->num_rows() >= 1)) : ?>
			...
			<table class="table table-bordered table-striped table_tr">
				...
				<tbody id="listObat">
					<?php if(isset($pembelian_umum_medicine)) : ?>
						...
						<?php endif;?>
						<?php foreach ($pembelian_umum_medicine->result() as $key => $value) : ?>
							<?php
							$aturan = explode(' ', $value->tdb_rule);
							if(is_array($aturan)){
							...
							<tr>
								<td style="text-align:center;">
									...
									<?php endif;?>
									<input type="text" id="medicine_<?=$key?>" name="medicine[recipe_medicine][]" class="recipe_medicine" value="<?=$value->tdb_item?>" <?=$readonly?>/>
								</td>
								...
								<td style="text-align:center;">
									<input type='text' class='input' name='medicine[qty][]' id='jumlah_<?=$key?>' style='width:20%;background:transparent;border: none;border-bottom: 1px dotted gray;text-align:center' value="<?=$value->tdb_qty?>" <?=$readonly?> >
									<input type="hidden" class="input price" name="medicine[price][]" id="price_<?=$key?>" value="<?=$value->tdb_price?>">
									<input type="hidden" class="input batch" name="medicine[batch][]" id="batch_<?=$key?>" value="<?=$value->tdb_batch?>">
									<span id='satuan_<?=$key?>' class='satuan'><?=$value->mtype_name?></span>
									<input type="hidden" name="recipe_summary[]" class="recipe_summary" id="recipe_summary_<?=$key?>" recipe_price="<?=$value->tdb_price?>" recipe_qty="<?=$value->tdb_qty?>" recipe_medicine="<?=$value->tdb_item?>" recipe_adm="0" ></input>
								</td>
								<td style="text-align:center;">
									<input type="text" class="input" name="medicine[note][]" id="note_<?=$key?>" style='background:transparent;border: none;border-bottom: 1px dotted gray;text-align:center' value="<?=$value->tdb_note?>" <?=$readonly?> ></input>
								</td>
							</tr>
							<script type="text/javascript">
							$('#medicine_'+incMedicine).select2({
								    ...
								    initSelection : function (element, callback) {
								        var data = {id: '<?=$value->tdb_item?>', text: '<?=$value->im_name?>'};
								        callback(data);
								    },
						            createSearchChoice:function(term, data) {
						                if ( $(data).filter( function() {
						        ...
						        $('#medicine_'+incMedicine).on("select2-selecting", function(e) {
						          	var url = "<?=base_url()?>apotek/resep_pasien/getMedicineDetail/"+(e.val);
						          	$.getJSON(url,(function(result){
					          			if( $('#medicine_'+incMedicine).select2('data') != null )
					          			{
					          				if( e.val == $('#medicine_'+incMedicine).select2('data').id )
						          			{
							          			if( (result.istok_item_price != null) )
						          					harga = result.istok_item_price;
						          				else
						          				...
						          				if( ((result.kel_name) != null) && (result.kel_name == 'Tuslah') )
						          					...
						          					recipe_adm = 0; // pembelian umum tanpa tusla

					          					$("#satuan_"+incMedicine).html(result.mtype_name);
						          				$("#price_"+incMedicine).val(harga);		
						          				$("#batch_"+incMedicine).val(result.istok_batch);		
						          				$("#jumlah_"+incMedicine).val('');	
					          					$("#recipe_summary_"+incMedicine).attr("recipe_price",harga);	
					          					$("#recipe_summary_"+incMedicine).attr("recipe_qty",0);	
					          					$("#recipe_summary_"+incMedicine).attr("recipe_medicine",e.val);	
					          					$("#recipe_summary_"+incMedicine).attr("recipe_adm",recipe_adm);	
					          				}
				          					calculateAll();
						          		}
					                }));
