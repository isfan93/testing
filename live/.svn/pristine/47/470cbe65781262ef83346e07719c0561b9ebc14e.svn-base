...
<script type="text/javascript">
  function reloadTable(){
      var url_hasil="<?php echo base_url()?>master/data_class/loaddata"
  ...
  $(function(){
            ...
            if (r == true) {
                  ...
                  $(".black_loader").fadeIn(300).fadeOut(function(){
                    // console.log($(t).attr('href'));
                     $.ajax({
                        url: "<?php echo base_url()?>master/data_class/delete_list",
      ...
      $.ajax({
        url: '<?php echo base_url()?>master/data_class/loaddata',
        type: "POST",
        crossDomain: true,
      ...
      $('.submit').click(function(){
          isvalid = $("form.tambahData").validate({
            ...
            submitHandler: function(form) {                
                ...
                $(".black_loader").fadeIn(300).fadeOut(function(){
                  $.post( "<?=base_url()?>master/data_class/create",formdata).done(function(){
      ...
      $('.submitEdit').live("click",function(){
          t = this;
          ins_id =  $(this).parent().parent().parent().find('.class_id').val();
          ins_name =  $(this).parent().parent().parent().find('.dsim_name').val();
          ins_price =  $(this).parent().parent().parent().find('.dsim_price').val();
          crsf = $('.crsf').val();
          url = "<?=base_url()?>master/data_class/update";
          datapost = "csrf_jike_2012="+crsf+"&ds[class_name]="+ins_name+"&ds[class_id]="+ins_id+"&ds[price]="+ins_price;
    ...
    <form class="stdform form-horizontal tambahData" style="display:none" method="post" >
      ...
      <div class="span11">
        <div class="widget-box">
          ...
          <div class="controls">
            <input type="text" autofocus="" name="ds[class_name]" id="class_name" placeholder="Nama Kelas">
          </div>
			<label class="control-label">Tarif</label>
			  <div class="controls">
				<input type="text" autofocus="" name="ds[price]" id="price" placeholder="Tarif">
			  </div>
        ...
        <div class="widgetbox">
            ...
            <?=form_open(cur_url().'search',array('class' => 'form-horizontal frm_search stdform','id' => 'form')); ?>
              <div class="chatsearch" >
                  <input type="text" class='searchname' name="nama_dokter" placeholder="Cari Kelas" style="wi dth:91%;margin:auto;">
              </div>
