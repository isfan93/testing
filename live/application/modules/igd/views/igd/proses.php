        ...
        $('.confirmdaftarranap').click(function(){
            ...
            if(rooms != ''){
                     $.post(url,data,function(response){
                        ...
                        if ("success" === response.success) {
                            $('.row-fluid:eq(0)').show('fast',function(){
                                $('#warning').removeClass().addClass('alert alert-success').html('Proses pendaftaran Rawat Inap berhasil. Silahkan akhiri (selesaikan) pemeriksaan IGD.').fadeIn().delay(2000).fadeOut(function(){
                                    // $('.row-fluid:eq(0),#warning').hide('fast');
                                    window.location = "<?=cur_url(-1)?>";
                                });
                            });
