...
<script type="text/javascript">
  function reloadTable(){
      var url_hasil="<?php echo base_url()?>master/data_lab_treathment_detail/loaddata"
      $(".list_askes").load(url_hasil);
  }

  function resetInput(){
      $('#dsd_name').val('');
      $("select[name='ds[dsd_gol]']").val('');
      $('#dsd_satuan').val('');
      $('#svm').val('');
	  $('#svf').val('');
  }
  $(function(){
            ...
            if (r == true) {
                  ...
                  $(".black_loader").fadeIn(300).fadeOut(function(){
                    // console.log($(t).attr('href'));
                     $.ajax({
                        url: "<?php echo base_url()?>master/data_lab_treathment_detail/delete_list",
      ...
      $('.list_askes').children().remove();
      $.ajax({
        url: '<?php echo base_url()?>master/data_lab_treathment_detail/loaddata',
        type: "POST",
        crossDomain: true,
      ...
      $('.submit').click(function(){
          isvalid = $("form.tambahData").validate({
            ...
            submitHandler: function(form) {                
                ...
                $(".black_loader").fadeIn(300).fadeOut(function(){
                  $.post( "<?=base_url()?>master/data_lab_treathment_detail/create",formdata).done(function(){
      ...
      $('.submitEdit').live("click",function(){
		  ...
		  ins_svm =  $(this).parent().parent().parent().find('.dsim_svm').val();
		  ins_svf =  $(this).parent().parent().parent().find('.dsim_svf').val();
		  ins_sat =  $(this).parent().parent().parent().find('.dsim_sat').val();
		  ins_gol =  $(this).parent().parent().parent().find('.dsim_gol').val();
          
          crsf = $('.crsf').val();
          url = "<?=base_url()?>master/data_lab_treathment_detail/update";
          datapost = "csrf_jike_2012="+crsf+"&ds[dsd_name]="+ins_name+"&ds[dsd_id]="+ins_id+"&ds[dsd_standart_value_male]="+ins_svm+
						"&ds[dsd_standart_value_female]="+ins_svf+"&ds[dsd_satuan]="+ins_sat+"&ds[dsd_gol]="+ins_gol;
    ...
    <form class="stdform form-horizontal tambahData" style="display:none" method="post" >
      ...
      <div class="span11">
        <div class="widget-box">
          ...
          <div class="controls">
            <input type="text" autofocus="" name="ds[dsd_name]" id="dsd_name" placeholder="Nama Tindakan">
          </div>
		  <label class="control-label">Nilai Standart Laki-laki</label>
          <div class="controls">
            <input type="text" autofocus="" name="ds[dsd_standart_value_male]" id="svm" placeholder="Nilai Standart">
          </div>
		  <label class="control-label">Nilai Standart Wanita</label>
          <div class="controls">
            <input type="text" autofocus="" name="ds[dsd_standart_value_female]" id="svf" placeholder="Nilai Standart">
          </div>
		  <label class="control-label">Satuan</label>
          <div class="controls">
            <input type="text" autofocus="" name="ds[dsd_satuan]" id="dsd_satuan" placeholder="Satuan">
          </div>
		  <label class="control-label" >Golongan</label>
			 <div class="controls">
				 <select name='ds[dsd_gol]' required id='dsd_gol' >
					<option value="">-- Pilih --</option>
					<?php foreach($golongan as $rows):?>
						<option value="<?=$rows->id?>"><?=$rows->golongan_name?></option>
					<?php endforeach;?>
        ...
        <div class="widgetbox">
            ...
            <?=form_open(cur_url().'search',array('class' => 'form-horizontal frm_search stdform','id' => 'form')); ?>
              <div class="chatsearch" >
                  <input type="text" class='searchname' name="nama_dokter" placeholder="Cari Data Tindakan Lab" style="wi dth:91%;margin:auto;">
              </div>
  ...
  <div class="row-fluid">
    <div class="span12">
      ...
      <div class="black_loader">
        ...
        <div class="row-fluid list_askes">
